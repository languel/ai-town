...
/** Ported from `convex/util/geometry.ts` — pure math, safe to use anywhere. */
...
export function orientationDegrees(vector: Vector): number {
  if (Math.sqrt(vector.dx * vector.dx + vector.dy * vector.dy) < EPSILON) {
    throw new Error(
      `Can't compute the orientation of too small vector ${JSON.stringify(vector)}`,
    );
  }
...
export function compressPath(densePath: PathComponent[]): Path {
  ...
  let candidate: PathComponent | undefined;
  for (const point of densePath.slice(1)) {
    if (!candidate) {
      candidate = point;
      continue;
    }
    // We can skip `candidate` if it interpolates cleanly between `last` and `point`.
